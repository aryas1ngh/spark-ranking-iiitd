...
- **Dependencies patched, and the Django pin now matches what deploys.** `6.0.6` → **`5.2.17`**. Three advisories affected both branches and were fixed in 5.2.16 and 6.0.7 together, so this is the patched version — but on the LTS line, which supports Python 3.10 upward. The 6.0 series requires Python 3.12+, which the deployment target does not provide, so the previous pin could never actually be installed there; `pip install -r` failed outright and the running version was whatever happened to be present. Verified on Python 3.13 and 3.14: 79 tests pass, `check --deploy` clean. `defusedxml` added. `pip-audit` reports no known vulnerabilities across the pinned set.
